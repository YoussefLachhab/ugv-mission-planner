...
def _interp_points(wps: Sequence[Waypoint], dt: float, max_speed: float) -> list[tuple[float, float, float]]:
    pts: list[tuple[float, float, float]] = []
    if not wps:
        return pts
    for a, b in zip(wps[:-1], wps[1:], strict=False):
...
def compliance_report(map_grid, avoid_zones, waypoints, max_speed) -> Report:
    ...
    path_len = sum(math.hypot(b.x - a.x, b.y - a.y) for a, b in zip(waypoints[:-1], waypoints[1:], strict=False))
